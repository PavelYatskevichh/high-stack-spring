...
if [[ $cnpg_flag && -z $(kubectl get pod -l app.kubernetes.io/name=cloudnative-pg --all-namespaces) ]]; then
...
if [[ $strimzi_flag && -z $(kubectl get pod -l name=strimzi-cluster-operator --all-namespaces) ]]; then
